#!/usr/bin/env node
// ollama-mods-proxy: speaks Ollama's *native* API to local clients, adds the
...
// cannot talk to Ollama Cloud directly. This proxy supplies both, and — like
// its sibling ollama-anthropic (see that shim's own history) — treats a
// non-loopback upstream override as a key leak rather than a config choice.

import http from "node:http";
import { Readable } from "node:stream";

// PROXY_UPSTREAM exists so the tests can point this at a fake upstream. Every
// forwarded request carries the Ollama Cloud key in an Authorization header,
// so an override that reached a remote host would hand the key to it;
// loopback only, and a violation is fatal rather than silently ignored. Ported
// from ollama-anthropic's own resolveUpstream after that proxy needed the same
// guard for the same key.
const UPSTREAM = resolveUpstream(process.env.PROXY_UPSTREAM);

function resolveUpstream(override) {
  if (!override) return "https://ollama.com";
  let host;
  try {
...
server.listen(PORT, "127.0.0.1", () => {
  ...
  log(`ollama-mods-proxy listening on http://127.0.0.1:${port} -> ${UPSTREAM} (think=${THINK})`);
});
